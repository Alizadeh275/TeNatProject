...
function get_graph_level(form_id) {
    level_selector = 'form#' + form_id + ' select#level';
    return $(level_selector).val();
}

function update_table_header(form_id, form_class) {

    if (form_class == 'graph_construction') {
        thead_selector = 'table#'.concat(form_id) + ' thead tr';
        $(thead_selector).children().remove();
...
function update_table_data(res, form_class, form_id) {
    ...
    update_table_header(form_id, form_class);
